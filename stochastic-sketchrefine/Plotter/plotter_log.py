...
pf_volatility_hardness = [1.78, 2.34, 2.46, 3.03, 3.54]

tpch_volatility_dir = 'TPC-H/Volatility'
tpch_volatility_methods = ['SummarySearch', 'RCLSolve']
tpch_volatility_hardness = [1.94, 2.86, 3.27, 3.42, 3.65]
...
for query in queries:
    ...
    axs[0, qcnt].set_ylim(0.1, 4500)
    axs[0, qcnt].set_xlim(min(x), max(x))
    axs[0, qcnt].set_title('Portfolio Q' + str(qcnt+1) + ' (H \u2248 ' + str(round(pf_scale_hardness[qcnt], 2)) + ')')
    if qcnt == 0:
        axs[0, qcnt].set_ylabel('Runtime (secs)', fontsize=15)
    methodcnt = 0
...
for query in queries:
    matplotlib.rcParams.update({'font.size': 15})
    axs[1, qcnt].set_ylim(0.1, 2000) 
    axs[1, qcnt].set_xlim(min(x), max(x))
    axs[1, qcnt].set_title('TPC-H Q' + str(qcnt+1) + ' (H \u2248 ' + str(round(tpch_scale_hardness[qcnt], 2)) + ')')
    if qcnt == 0:
        axs[1, qcnt].set_xlabel('No. of Tuples', fontsize=15)
        axs[1, qcnt].set_ylabel('Runtime (secs)', fontsize=15)
    else:
        axs[1, qcnt].set_xlabel('No. of Tuples', fontsize=15)
    methodcnt = 0
...
for query in queries:
    matplotlib.rcParams.update({'font.size': 15})
    axs[0, qcnt].set_ylim(top=2700/60) 
...
for query in queries:
    matplotlib.rcParams.update({'font.size': 15})
    axs[1, qcnt].set_ylim(top=1200/60) 
