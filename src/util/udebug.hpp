...
public:
	...
	double getTime(const string &label) const
	{
		...
		{
			return clocks.at(label).first;
		}
		return 0.0;
	}
	
	double getCurrentRuntime(const std::string &label) const
	{
		if (timePoints.count(label))
		{
			auto currentTime = Clock::now();
			return std::chrono::duration_cast<std::chrono::seconds>(currentTime - timePoints.at(label)).count();
		}
		return 0.0;
	}
};
